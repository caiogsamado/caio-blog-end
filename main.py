...
class Comment(db.Model):
    ...
    blogpost = db.Column(db.Integer, db.ForeignKey('blog_posts.id'))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        password_hash = generate_password_hash(form.password.data,
                                               ...
                                               salt_length=8)
        check_user = User.query.filter_by(email=email_entered).first()
        if check_user == None:
            new_user = User(
...
def show_post(post_id):
    ...
    commentaries = requested_post.comments
    if current_user.is_authenticated:
        user_id = current_user.id
    ...
    return render_template("post.html", post=requested_post, logged_in=current_user.is_authenticated, id=user_id, form=comment_form, comments = commentaries)
